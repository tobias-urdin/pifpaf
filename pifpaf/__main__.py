...
class RunGroup(click.MultiCommand):
    ...
    def get_command(self, ctx, name):
        ...
        return click.Command(
            name=name,
            callback=lambda *args, **kwargs: self._run(name, plugin, ctx, *args, **kwargs),
            params=params)
    ...
    def _run(self, daemon, plugin, ctx, command, **kwargs):
        debug = ctx.obj['debug']
        env_prefix = ctx.obj['env_prefix'].encode()
        global_urls_variable = ctx.obj['global_urls_variable'].encode()
        driver = plugin(env_prefix=env_prefix,
                        debug=debug,
                        **kwargs)

        daemon = daemon.encode()
